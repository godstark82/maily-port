...
const createEditorStore = (initProps?: Partial<EditorProps>) => {
  ...
  return createStore<EditorState>()((set) => ({
    ...
    setJson: (json) => {
      set(() => ({ json }));
      // Save to cookie
      Cookies.set('editor_content', JSON.stringify(json), { expires: 1 });
    },
    ...
    setState: (state) => {
      set(() => state);
      // Save json to cookie if it exists in state
      if (state.json) {
        Cookies.set('editor_content', JSON.stringify(state.json), { expires: 1 });
      }
    },
  }));
};
...
export function EditorProvider(props: EditorProviderProps) {
  ...
  // Initialize cookie with default content
  useEffect(() => {
    const defaultJson = defaultProps.json || storeRef.current?.getState().json;
    if (defaultJson) {
      Cookies.set('editor_content', JSON.stringify(defaultJson), { expires: 1 });
    }
  }, [defaultProps.json]);
